...
1. **Trafo Step-Up (Penaik Tegangan):**
   - **Vs > Vp** (Tegangan sekunder lebih besar dari primer)
   - **Ns > Np** (Lilitan sekunder lebih banyak dari primer)
   - **Is < Ip** (Arus sekunder lebih kecil dari primer)
...
2. **Trafo Step-Down (Penurun Tegangan):**
   - **Vs < Vp** (Tegangan sekunder lebih kecil dari primer)
   - **Ns < Np** (Lilitan sekunder lebih sedikit dari primer)
   - **Is > Ip** (Arus sekunder lebih besar dari primer)
...
| :--- | :--- | :--- | :--- |
| **Gaya Lorentz** | F = B × I × L | Energi Listrik -> Energi Gerak | Motor listrik, Kipas angin |
| **Induksi Faraday** | Perubahan Fluks Magnet | Energi Gerak -> Energi Listrik | Generator PLTA, Dinamo sepeda |
| **Trafo Step-Up** | Ns > Np, Vs > Vp | Penaik Tegangan AC | Transmisi jarak jauh SUTET |
| **Trafo Step-Down** | Ns < Np, Vs < Vp | Penurun Tegangan AC | Charger HP, Elektronik rumah |
...
cur.execute("UPDATE articles SET content = %s, updated_at = CURRENT_TIMESTAMP WHERE slug = %s", 
            (clean_content, 'kemagnetan-medan-magnet-dan-induksi-elektromagnetik'))

conn.commit()
cur.close()
conn.close()
print("Cleaned magnetism article typography successfully!")
